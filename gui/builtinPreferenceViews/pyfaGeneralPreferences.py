...
from service.settings import SettingsProvider, LocaleSettings, ImportSettings
import eos.config
...
class PFGeneralPref(PreferenceView):
    ...
    def populatePanel(self, panel):
        ...
        self.importSettings = ImportSettings.getInstance()
        ...
        mainSizer.Add(langBox, 0, wx.EXPAND | wx.TOP |
                      wx.RIGHT | wx.BOTTOM, 10)

        langSizer = wx.BoxSizer(wx.HORIZONTAL)

        self.langChoices = sorted([langInfo for lang, langInfo in LocaleSettings.supported_langauges(
        ).items()], key=lambda x: x.Description)
        pyfaLangsEnabled = bool(self.langChoices)

        if pyfaLangsEnabled:
            self.stLangLabel = wx.StaticText(panel, wx.ID_ANY, _t(
                "pyfa:"), wx.DefaultPosition, wx.DefaultSize, 0)
            self.stLangLabel.Wrap(-1)
            langSizer.Add(self.stLangLabel, 0, wx.ALL |
                          wx.ALIGN_CENTER_VERTICAL, 5)

            def langDisplay(langInfo):
                progress = self.localeSettings.get_progress(
                    langInfo.CanonicalName)
                progress_display = (" ({}%)".format(
                    progress['translated_progress']) if progress is not None else "")
                return langInfo.Description + progress_display

            self.chLang = wx.Choice(panel, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, [
                                    langDisplay(x) for x in self.langChoices], 0)
            self.chLang.Bind(wx.EVT_CHOICE, self.onLangSelection)

            selectedIndex = self.langChoices.index(next(
                (x for x in self.langChoices if x.CanonicalName == self.localeSettings.get('locale')), None))
        ...
        self.stEosLangLabel = wx.StaticText(panel, wx.ID_ANY, _t(
            "EVE Data:"), wx.DefaultPosition, wx.DefaultSize, 0)
        self.stEosLangLabel.Wrap(-1)
        eosLangSizer.Add(self.stEosLangLabel, 0, wx.ALL |
                         wx.ALIGN_CENTER_VERTICAL, 5)

        self.eosLangChoices = [(LocaleSettings.defaults['eos_locale'], LocaleSettings.defaults['eos_locale'])] + \
            sorted([(wx.Locale.FindLanguageInfo(x).Description, x)
                   for x in eos.config.translation_mapping.keys()], key=lambda x: x[0])

        self.chEosLang = wx.Choice(panel, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, [
                                   x[0] for x in self.eosLangChoices], 0)
        ...
        eosLangSizer.Add(self.chEosLang, 0, wx.ALL |
                         wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        self.cbRackSlots = wx.CheckBox(panel, wx.ID_ANY, _t(
            "Separate Racks"), wx.DefaultPosition, wx.DefaultSize, 0)
        mainSizer.Add(self.cbRackSlots, 0, wx.ALL | wx.EXPAND, 5)

        labelSizer = wx.BoxSizer(wx.VERTICAL)
        self.cbRackLabels = wx.CheckBox(panel, wx.ID_ANY, _t(
            "Show Rack Labels"), wx.DefaultPosition, wx.DefaultSize, 0)
        labelSizer.Add(self.cbRackLabels, 0, wx.ALL | wx.EXPAND, 5)
        mainSizer.Add(labelSizer, 0, wx.LEFT | wx.EXPAND, 30)

        self.cbShowTooltip = wx.CheckBox(panel, wx.ID_ANY, _t(
            "Show fitting tab tooltips"), wx.DefaultPosition, wx.DefaultSize, 0)
        mainSizer.Add(self.cbShowTooltip, 0, wx.ALL | wx.EXPAND, 5)

        self.cbGaugeAnimation = wx.CheckBox(panel, wx.ID_ANY, _t(
            "Animate gauges"), wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.cbOverwriteImportFits = wx.CheckBox(panel, wx.ID_ANY, _t(
            "Imported fits with same name should overwrite old fit"), wx.DefaultPosition, wx.DefaultSize, 0)
        mainSizer.Add(self.cbOverwriteImportFits, 0, wx.ALL | wx.EXPAND, 5)
        ...
        mainSizer.Add(self.rbAddLabels, 0, wx.EXPAND |
        ...
        self.cbRackSlots.SetValue(
            self.sFit.serviceFittingOptions["rackSlots"] or False)
        ...
        self.cbOverwriteImportFits.SetValue(
            self.importSettings.getShouldOverwite()
        )
        ...
        self.cbOverwriteImportFits.Bind(
            wx.EVT_CHECKBOX, self.onCBOverwriteImportFits)
    ...
    def onCBOverwriteImportFits(self, event):
        self.importSettings.setShouldOverwrite(
            self.cbOverwriteImportFits.GetValue())
